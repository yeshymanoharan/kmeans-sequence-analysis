...
class SequenceKMeans():
    ...
    '''
    def initialize(sequences_input, number_of_means, initial_centroids=None):
        ...
        if initial_centroids == None:
            ...
            for centroid in initial_centroids:
                ...
                intercohesion[centroid] = []
                aligned_centroids[centroid] = []

        print('init', str(len(sequences)))


    def group(names=None, debug=False):
        ...
        for index in range(len(sequences)):
            ...
            centroid_highest_similarity = 0
            for centroid in centroids.keys():
                #print(sequence, centroid)
                similarity, consensus_position = SmithWaterman.main(sequence, [x for x in centroid], debug=debug)
                #print(sequence, centroid)
                #print(similarity)
                '''
                for centroid_compare in centroids.keys():
                    if centroid_compare != centroid:
                        centroid_similarity, consensus_position = SmithWaterman.main([x for x in centroid_compare], [x for x in centroid])
                        if centroid_similarity > centroid_highest_similarity:
                            centroid_highest_similarity = centroid_similarity
                '''
            ...
            if names == None:
                ...
                centroids[best_centroid].append({'name':names[index][0], 'similarity':first_best_similarity_value, 'sequence':''.join(sequence)})
            #print(intracohesion)
            ...
            #intercohesion[best_centroid].append(centroid_highest_similarity)
    ...
    def test(sequences_input, names_input, initial_centroids, debug=False):
        global centroids
        SequenceKMeans.initialize(sequences_input, len(initial_centroids), initial_centroids)
        SequenceKMeans.group(names=names_input, debug=debug)
        return centroids
